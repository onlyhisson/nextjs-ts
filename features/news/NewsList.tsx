...
const news_detail = {
  ...
  content:
    "Investing.com - European stock markets are expected to open lower Friday as investors digest the rate hike guidance from the European Central Bank ahead of the release of key U.S. inflation data.<br/>At 2 AM ET (0600 GMT), the DAX futures contract in Germany traded 0.7% lower, CAC 40 futures in France dropped 0.7% and the FTSE 100 futures contract in the U.K. fell 0.7%.",
};
...
const NewsList = () => {
  ...
  const renderedNews = newList.map((el: INews, idx: number) => (
    <NewsItem key={idx} active={el.id === postId}>
      {el.id !== postId && (
      ...
      {el.id === postId && (
        <NewsDetail news={news_detail} onClick={() => setPostId("0")} />
      )}
    </NewsItem>
  ));
  return (
