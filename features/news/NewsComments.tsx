...
const comments = [
  {
    ...
    content:
      "U.S. oil producers profiting from sky-high prices are doling out billions to shareholders and building cash reserves, a strategy irking lawmakers an",
    good: "10",
    bad: "5",
    user: {
      name: "Jang",
  ...
  },
  {
    ...
    content:
      "With the Autocopy feature, you can access and start copying thousands of active real-money traders with a few simple clicks! Follow trades with a fixed investment or copy their trades in percentages, and enjoy a fully automated trade opening & closing by the trade-master!",
    good: "1",
    bad: "0",
    user: {
      name: "Kim",
...
const NewsCommentBox = ({ comment }: any) => {
  ...
  return (
    <CommentBox key={id} reply={reply.replyId != null}>
      <CommentBoxLeft>
        <img src={user.img ? user.img : `/images/user.png`} alt="user" />
      </CommentBoxLeft>
      <CommentBoxRight>
...
const NewsComments = ({ newsId }: TNewsIdProps) => {
  const nComments = comments.filter(
    (el) => el.newsId === newsId && el.reply.replyId === null
  );
  const renderedItem = nComments.map((comment, idx) => {
    return (
      <div key={idx}>
        <NewsCommentBox comment={comment} />
        {comment.reply.items.length > 0 &&
          comment.reply.items.map((el2, idx2) => {
